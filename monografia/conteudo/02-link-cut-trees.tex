...
Logo, fica claro que o consumo de tempo dessa rotina é algo proporcional ao número de iterações realizadas vezes o custo das chamadas que operam sob os caminhos preferidos. Em particular, \citet{demaine_holmgren_jian_stepanenko_ishaque} usam a técnica de \emph{heavy-light decomposition} para mostrar que, em uma sequência de $m$ operações \texttt{access}, são realizadas $\Oh(m \log n)$ iterações. Ademais, eles mostram que as chamadas realizadas a cada iteração custam $\Oh(1)$ amortizado. Portanto, a rotina \texttt{access} consome tempo $\Oh(log n)$ amortizado.
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        ...
        \WhileNot  {\emph{u.is\_root()}} \Comment{$u$ não é raiz da LCT e nem da Splay}
        \State \emph{parent $\gets$ u.parent}
        \State \emph{grandparent $\gets$ parent.parent}
        \IfNot{\emph{parent.is\_root()}}
