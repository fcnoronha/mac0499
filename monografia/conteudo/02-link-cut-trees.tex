...
Neste capítulo, apresentaremos as \emph{link-cut tree}, introduzida por ~\citet{10.1145/800076.802464}. Esta estrutura de dados serve como base para as estruturas retroativas apresentadas nos próximos capítulos.

%% ------------------------------------------------------------------------- %%
\section{Ideia}
\label{sec:lct-ideia}

As \emph{link-cut trees} são uma estrutura de dados que nos permite manter uma floresta de árvores enraizadas com peso nas arestas, onde os nós de cada árvore possuem um número arbitrário de filhos. Ademais, essa estrutura nos fornece o seguinte conjunto de operações:
...
\begin{itemize}
    ...
    \item \texttt{split(u)}: separa o caminho preferido que contém o nó $u$ em dois, quebrando a conexão entre $u$ e seu filho preferido, caso exista. Ao final, tanto $u$ quanto o seu filho preferido inicial serão os identificadores de seus caminhos preferidos.
...
Com tal conjunto de funções, podemos avançar para os métodos das \emph{link-cut trees}.

\subsection{Rotina Access}
\label{subsection:lct-access}

Uma rotina utilizada por todos os métodos das \emph{link-cut trees} que vamos implementar é a \texttt{access(u)}. A partir dela conseguimos reorganizar a estrutura interna de uma árvore representada a nosso favor. Basicamente, a operação \texttt{access(u)} cria um caminho preferido que parte de $u$ e vai até a raiz da árvore representada de $u$. Com isso, todas as arestas preferidas que tinham somente uma das pontas fazendo parte deste novo caminho deixam de ser preferidas e $u$ termina sem nenhum filho preferido.
...
Com estes valores em mãos, podemos ir criando um caminho preferido através de sucessivas concatenações, unindo a parte superior do caminho que \texttt{current\_root} identifica ao caminho mantido por \texttt{last}. Ao final dessa concatenação, temos que \texttt{current\_root} é o identificador deste caminho que está sendo construído, e após guardarmos seu valor em \texttt{last}, podemos prosseguir para o próximo passo, onde \texttt{current\_root} agora corresponde ao nó imediatamente em cima do caminho preferido que estamos construindo.

\begin{algorithm}[h!]
    \caption{Rotina Access}\label{lct:access}
    \begin{algorithmic}
        \Function{access}{\emph{u}}
        \State \emph{last $\gets$ NULL}
        \State \emph{current\_root $\gets$ u}
        \State \Comment{concatena todos os caminhos preferidos de $u$ até a raiz da árvore representada}
        \While{\emph{current\_root $\neq$ NULL}}
        \State \emph{make\_identifier(current\_root)}
        \State \Comment{concatena a parte superior do caminho preferido de $u$ ao caminho de \texttt{last}}
        \State \emph{join(current\_root, last)}
        \State \emph{last $\gets$ current\_root}
        \State \emph{current\_root $\gets$ get\_parent\_path\_node(current\_root)}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        ...
        \Function{link}{\emph{u, v, w}}
        \State \emph{uv\_edge $\gets$ new Node(w)} \Comment{cria nó com peso $w$, representando a aresta}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        \Function{is\_connected}{\emph{u, v}}
        \State \emph{access(u)}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        ...
        \Function{maximum\_edge}{\emph{u, v}}
        \State \emph{make\_root(v)}
        \State \emph{access(u)}
...
Assim, encerramos a explicação da implementação dos métodos das \emph{link-cut trees}.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{A árvore em (a) mostra os valores do booleano \texttt{is\_reversed} acima do rótulo de cada no. Já a árvore em (b) mostra a splay tree resultante após todas as inversões serem resolvidas.}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        \Function{Splay}{\emph{u}}
        \While{\emph{$\neg$u.is\_root()}} \Comment{$u$ não é raiz da LCT e nem da Splay}
        ...
        \If{\emph{(grandparent.r\_child = parent) = (parent.r\_child = u)}}
        \State \emph{rotate(parent)} \Comment{zig-zig ou zag-zag}
        \Else
        \State \emph{rotate(u)} \Comment{zig-zag}
        \EndIf
        \EndIf
        \State \emph{rotate(u)}
        \EndWhile
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        ...
        \Function{revese\_path}{\emph{u}}
        \State \emph{u.is\_reversed $\gets \neg$u.is\_reversed}
        \State \emph{u.push\_reversed\_bit()} \Comment{inverte os filhos de $u$ e propaga a inversão do bit}
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        \Function{get\_path\_end\_node}{\emph{u}}
        \State \emph{splay(u)}
        \State \emph{smallest\_value $\gets$ u}
        \While{\emph{smallest\_value.l\_child $\neq$ NULL}}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        \Function{get\_parent\_path\_node}{\emph{u}}
        \State \emph{splay(u)}
...
\begin{algorithm}[h!]
    ...
    \begin{algorithmic}
        \Require{$u$ identificador de seu caminho preferido}
        \Function{get\_maximum\_path\_value}{\emph{u}}
