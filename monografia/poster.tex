...
\begin{frame}[fragile]\centering
  \begin{columns}[T]
    ...
    \begin{block}{Link-Cut tree}
      As link-cut trees são uma estrutura de dados que nos permite manter uma \textbf{floresta de árvores enraizadas com peso nas arestas}, onde os vértices de cada árvore possuem um número arbitrário de filhos. Ademais, a floresta armazenada por essa estrutura não é orientada --- isto é, suas arestas não possuem uma direção --- e devido à maneira que ela é usada para nas implementações, sua raiz é constantemente redefinida, de modo que perdemos o arranjo original das árvores.
      ...
      \begin{shaded}
        \textbf{Ideia:} Dividir a floresta em caminhos vértice-disjuntos, chamados \emph{caminhos preferidos}. Cada um desses caminhos é representado na forma de uma \textit{splay tree}, uma estrutura que nos permite unir e quebrar estes caminhos de forma bastante eficiente.
      \end{shaded}
    ...
    \begin{exampleblock}
      ...
      A link-cut tree nos fornece o seguinte conjunto de operações:
    ...
    \begin{block}{Floresta geradora mínima retroativa}
      ...
      A \textbf{floresta geradora mínima incremental} tem a seguinte interface:
      \definecolor{shadecolor}{rgb}{0.93, 0.80, 0.82} % pale pink
      \begin{shaded}
        \begin{itemize}
      ...
      \definecolor{shadecolor}{rgb}{0.74, 0.83, 0.9} % pale blue
      \begin{shaded}
        \textbf{Ideia:} Organizar cada operação retroativa de inserção numa lista ordenada pelo instante de tempo em que a aresta foi inserida. Em seguida, utilizar a técnica de \textit{square-root decomposition} para dividir essa lista em $\sqrt{m}$ blocos, onde $m$ é o número total de operações na lista. Essa divisão vai sendo refeita conforme novas operações de inserção vão sendo adicionadas, a fim de manter o tamanho dos blocos aproximadamente constante. Por último, distribuir as operações de cada bloco em diferentes florestas geradoras mínimas incrementais, fazendo com que uma consulta acerca do instante de tempo $t$ possa ser realizada de maneira eficiente por uma estrutura que contenha o grafo próximo ao instante $t$.
      \end{shaded}
...
% falar do paper
% arrumar texto figura 3
% destacar algumas partes do texto
% revisar
