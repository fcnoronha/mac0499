...
\begin{abstract}
  ...
  persistent version of the data structure in question.  We focus on the case in 
...
where the respective data might be manipulated in imperfect ways, and once in a while 
...
of the data structure in question, they show how to achieve a more effective space
...
This leads to an amortized query and edge addition time of $\Oh(\sqrt{m}\lg n)$ in a graph
with $n$ vertices. The space used by their implementation is $\Theta(m\sqrt{m})$ 
...
at most~$(3/2)\sqrt{m}$ operations occur between consecutive checkpoints, 
with~$m$ changing during the process as the operations happen. 
The second part of the proof is concerned with saving space, and 
...
operations occur between consecutive checkpoints, again with a variable $m$.
...
the data structure in question to apply the space saving strategy from Theorem~5 
...
version of the data structure in question.  
...
Our strategy follows the same idea of Demaine et al., but it does not 
rely on the use of a persistent version of the data structure in question. 
...
as expected.  The operation \texttt{add\_edge}$(u,v,w,t)$ is performed 
...
considered is incremental, a fully retroactive version of the problem requires 
the ability to remove a previous addition from any point in the update sequence,
a capability their implementation lacks.

Note that a version allowing for the removal of edge additions should not be 
confused with an implementation of a retroactive dynamic MSF. A dynamic structure 
for an MSF not only allows for the maintenance of edge additions but also edge 
removals in the update sequence.
...
However, since the data structure $D_i$ is for incremental MSF, we do not have support 
to edge removals.

The approach we presented to avoid the need for a fixed $m$, as seen in de Andrade Junior 
and Seabra's implementation, demonstrates an interesting method for adapting a square-root
decomposition technique to our advantage. This technique could be further explored in other 
applications as well.

Finally, the algorithm of Holm, de Lichtenberg, and Thorup~\cite{HolmLT2001} maintains
...
The first author would like to thank the support from Conselho Nacional de Desenvolvimento Científico e Tecnológico (CNPq) of Brazil, grants 310979/2020-0 and 404315/2023-2, and Fundação de Amparo à Pesquisa do Estado de São Paulo (FAPESP), grant 2023/03167-5.
