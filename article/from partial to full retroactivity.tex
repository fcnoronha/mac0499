...
% \title[Faster rebuilding in the (semi-)retroactive MSF]
% {Faster rebuilding in the \\ (semi-)retroactive minimum spanning forest}
...
% So, to perform a forgotten update that should have been done in the past, or to undo an update previously done by mistake, 
% seems to requires to keep track of the updates and of the order in which they are performed. 
% the sequence is sorted by these time stamps.
% Retroactivity takes into account the moment at which operations are perfomed. 
...
Our initial goal was to improve on their implementation, allowing the length 
...
of Demaine et al.~\cite{DemaineIL2007}.  However, to achieve the same time 
...
There is a sophisticated functional implementation of link-cut trees described in 
the literature~\cite{DemaineLP2008}, based on the use of the so called \emph{fingers}. 

Instead of following this approach, we came up with a simple way to use 
...
% Descrição da reconstrução
% Prova da correção
% Análise do tempo e espaço
...
% Descrição da solução apresentada por Andrade
% qual a ED usada
% qual o consumo de tempo das operações suportadas na implementação
% Nossa motivação para tentar eliminar as restrições e consequente desenvolvimento dessa nova approach
% Explicação de como utilizar a nova ideia para solucionar o problema
...
on the incremental MSFs.  As argue in Subsection~\ref{subsec:semi}, 
...
% Next steps: to expand the implementation to allow for cancelation of edge additions. 
% What is involved?

% This is still different from a fully retroactive dynamic MSF. 
% What is missing towards this? 
