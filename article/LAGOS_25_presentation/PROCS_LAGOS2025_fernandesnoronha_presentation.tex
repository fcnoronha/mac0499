...
\author[Felipe C. Noronha]{Cristina G. Fernandes, Felipe C. de Noronha}
\institute[IME-USP]{
    IME-USP -- Brazil
}
...
\begin{frame}
    ...
    \item \textbf{Link-cut tree operations:}
        ...
        \item \texttt{is\_connected}$(u,v)$: $\Oh(\log n)$ amortized
        \end{itemize} \vfill\pause
    \item \textbf{Algorithm for adding edge $(u,v,w)$:}
        \begin{enumerate}
        \item Check if $u$ and $v$ are in connected in the same component
        \item If not: add edge $(u,v,w)$ to forest
        \item If yes: find the edge with maximum cost on the $u$-$v$ path
        \item If $w < \text{maximum cost}$: replace maximum cost edge with $(u,v,w)$
        \end{enumerate} \vfill\pause
    \item \textbf{Total cost:} Amortized $\Oh(\log n)$ per edge addition \vfill
    \end{itemize}
    \note[item]{So, what was Frederickson's solution? He showed that link-cut trees can efficiently maintain this incrementing forest. ---------- SKIP SLIDE}
    % \note[item]{---------- SKIP SLIDE ----------}
    \note[item]{Link-cut trees provide all the operations we need, all in $O(\log n)$ amortized time: $find\_max$ to find the most expensive edge on a path, $link$ to add a weighted edge, $cut$ to remove one, and $is\_connected$ to check if $u$ and $v$ are in the same component}
    \note[item]{---------- SKIP SLIDE ----------}
    ...
    \note[item]{If our new edge's cost is cheaper than that maximum cost, we swap them: we $cut$ the old, expensive edge and $link$ our new, cheaper edge .}
    \note[item]{---------- SKIP SLIDE ----------}
...
\begin{frame}
    ...
    \begin{itemize}
    \item \textbf{add\_edge(g, h, 4):} Add edge $(g,h)$ with cost 4 \vfill
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 3 (Cycle Check)}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.8]
            ...
            % Edges from fast-forwarded steps (1-4)
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (G) -- (H) node[midway, below=0.1cm, font=\small] {4};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (C) -- (A) node[midway, left=0.1cm, font=\small] {1};
            ...
            % New edge being checked (dotted)
            \draw[blue, thick, dotted] (C) -- (F) node[midway, above=0.1cm, font=\small] {5};  % CF
    ...
    \note[item]{After fast-forwarding through a few steps (adding f-g and a-f), our forest is more connected.}
    \note[item]{Now, we add (c, f) with cost 5. This is our first interesting case.}
...
%-------------------------------------------------
% Incremental MSF Example - Step 3 (Result)
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 3 (Result)}
    
    \begin{itemize}
    \item \textbf{add\_edge(c, f, 5):} Edge cost is greater than max cost (2), not added \vfill
    \end{itemize}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.8]
           ...
           % Row 1: A B
            ...
            \node[circle, draw, fill=blue!20, minimum size=0.6cm] (H) at (4, 0) {h};
            % Edge NOT in MSF
            \draw[blue, thick] (C) -- (F) node[midway, above=0.1cm, font=\small] {5};
            % CF

            % Edges 1-4 (from prev steps)
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (G) -- (H) node[midway, below=0.1cm, font=\small] {4};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (C) -- (A) node[midway, left=0.1cm, font=\small] {1};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (F) -- (G) node[midway, below=0.1cm, font=\small] {6};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (A) -- (F) node[midway, left=0.1cm, font=\small] {2};
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 5 (Cycle Check)}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.8]
            ...
            % New edge being checked (dotted)
            \draw[blue, thick, dotted] (A) -- (D) node[midway, left=0.1cm, font=\small] {3};
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 5 (Result)}
    
    \begin{itemize}
    \item \textbf{add\_edge(a, d, 3):} Cost 3 < max cost 7, swap edges \vfill
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 6 (Cycle Check)}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.8]
            ...
            % Edge NOT in MSF
            \draw[blue, thick] (C) -- (F) node[midway, above=0.1cm, font=\small] {5};
            \draw[blue, thick] (F) -- (D) node[midway, above=0.1cm, font=\small] {7};
            
            % Edges 1-7 (state from end of last step)
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (G) -- (H) node[midway, below=0.1cm, font=\small] {4};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (C) -- (A) node[midway, left=0.1cm, font=\small] {1};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (F) -- (G) node[midway, below=0.1cm, font=\small] {6};
            \draw[red, thick, decorate, decoration={snake, amplitude=1pt, segment length=8pt}] (A) -- (F) node[midway, left=0.1cm, font=\small] {2};
            ...
            % New edge being checked (dotted)
            \draw[blue, thick, dotted] (D) -- (G) node[midway, right=0.1cm, font=\small] {2};
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 6 (Result)}
    
    \begin{itemize}
    \item \textbf{add\_edge(d, g, 2):} Cost 2 < max cost 6, swap edges \vfill
    \end{itemize}
...
%-------------------------------------------------
\begin{frame}
    \frametitle{Incremental MSF example - Step 7 (Final Result)}
    ...
    \note[item]{---------- SKIP SLIDE ----------}
    \note[item]{...we would eventually arrive at the final, optimal Minimum Spanning Tree. The one shown here, for example, has a total cost of 14.}
...
\begin{frame}
    ...
    \begin{enumerate}
        ...
        \item For $i = 1$ to $k+1$:
        \begin{itemize}
        \item $p \gets$ \textsc{kth}$(S, i(k+1))$ \hfill {\footnotesize $\rhd$ $i(k+1)$th item in the sequence of updates}
