...
\begin{frame}
    ...
    \note[item]{Walk through the algorithm step by step:}
    ...
    \note[item]{Show how cycle detection and edge replacement works using link-cut tree properties}
...
\begin{frame}
    ...
    \note[item]{Now we have components: {g,h,f,a,c} - all vertices connected!}
...
\begin{frame}
    ...
    \note[item]{Link-cut tree operations: find\_max\(c,f\) finds edge \(f,g\) with cost 6}
    \note[item]{Since new edge cost 5 $<$ max cost 6, we replace \(f,g\) with \(c,f\)}
    \note[item]{Current MSF: \{g-h, c-a, c-f, a-f\} with total cost 12 (improved!)}
...
\begin{frame}
    ...
    \note[item]{Link-cut tree operations: find\_max\(f,d\) finds edge \(g,h\) with cost 4}
    \note[item]{Since new edge cost 7 > max cost 4, we don't replace - edge is rejected}
    \note[item]{Current MSF: \{g-h, c-a, c-f, a-f\} with total cost 12 \(unchanged\)}
    \note[item]{This shows how expensive edges are rejected to maintain optimality}
    \note[item]{Key insight: not all edges improve the MSF - we only keep beneficial ones}
...
\begin{frame}
    ...
    \note[item]{This creates a cycle! a-d-f-c-a forms a cycle}
    \note[item]{Link-cut tree operations: find\_max\(a,d\) finds edge \(c,f\) with cost 5}
    \note[item]{Since new edge cost 3 < max cost 5, we replace \(c,f\) with \(a,d\)}
    \note[item]{Current MSF: \{g-h, c-a, a-d, a-f\} with total cost 10 \(improved!\)}
...
\begin{frame}
    ...
    \note[item]{This creates a cycle! d-g-h-f-a-d forms a cycle}
    \note[item]{Link-cut tree operations: find\_max\(d,g\) finds edge \(g,h\) with cost 4}
    \note[item]{Since new edge cost 2 < max cost 4, we replace \(g,h\) with \(d,g\)}
    \note[item]{Current MSF: \{d-g, c-a, a-d, a-f\} with total cost 8 \(improved!\)}
...
\begin{frame}
    ...
    \note[item]{Give concrete example: MSF at time t = 5 vs MSF at time t = 10}
