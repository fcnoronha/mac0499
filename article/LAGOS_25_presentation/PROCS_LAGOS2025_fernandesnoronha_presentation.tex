...
\begin{frame}
    ...
    % \note[item]{---------- SKIP SLIDE ----------}
    \note[item]{Link-cut trees provide all the operations we need, all in $O(\log n)$ amortized time: $find\_max$ to find the most expensive edge on a path, $link$ to add a weighted edge, $cut$ to remove one, and $is_connected$ to check if $u$ and $v$ are in the same component}
    \note[item]{---------- SKIP SLIDE ----------}
...
\begin{frame}
    ...
    \begin{itemize}
    \item \textbf{add\_edge(c, f, 5):} Edge is > max cost (2), not added \vfill
    \end{itemize}
